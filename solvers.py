...
def _rk4_step_jit(state, eps, delta, eta, duffing, is_vdp, dt):
...
@numba.njit(fastmath=True)
def _numba_integrate_chunk(state, eps, delta, eta, duffing, is_vdp, t_start, t_end, dt, n_points):
    ...
    for step in range(1, total_steps + 1):
        curr_s = _rk4_step_jit(curr_s, eps, delta, eta, duffing, is_vdp, actual_dt)
    ...
    while t_curr < max_time:
        ...
        if method == "numba_rk4":
            t_chunk, y_chunk = _numba_integrate_chunk(
                state, eps, delta, eta, duffing, is_vdp, t_curr, t_next, rk4_dt, points_per_chunk
            )
        else:
...
def solve(
    ...
    """
    # התאמת קלט: תמיכה גם בהעברת נקודה בודדת [y1, v1, y2, v2]
...
if __name__ == "__main__":
    ...
    # 1. פתרון עבור נקודה אחת (מועברת כרשימה):
    print("--- 1. פתרון עבור נקודה אחת ---")
    res_single = solve(eq, [[2.0, 0.0, 0.0, 0.0]], method="numba_rk4")
    print(f"תוצאה לנקודה אחת: התכנס ב-t={res_single[0].convergence_time:.1f}, שיפוע={res_single[0].slope:.2e}")

    # 2. פתרון עבור 8 נקודות במקביל על 8 מעבדים:
    print("\n--- 2. פתרון עבור 8 נקודות במקביל (workers=8) ---")
    points = [[y1, 0.0, 0.0, 0.0] for y1 in np.linspace(0.5, 4.0, 8)]
    t0 = time.perf_counter()
    res_batch = solve(eq, points, workers=8, method="numba_rk4")
    t_total = time.perf_counter() - t0
    print(f"הסתיים תוך {t_total:.4f} שניות עבור {len(res_batch)} נקודות.")
